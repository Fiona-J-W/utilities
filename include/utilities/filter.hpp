...
#ifndef UTILITIES_FILTER_RANGE_HPP
#define UTILITIES_FILTER_RANGE_HPP
...
public:
	...
	public:
		...
		using difference_type = std::ptrdiff_t;
		iterator(const filter_range& range, Filter filter, base_iterator_type it):
			m_range{&range}, m_filter(filter), m_it{std::move(it)} {}
		iterator() = delete;
		reference operator*() const {return *m_it;}
		pointer operator->() const {return m_it.operator->();}
		iterator& operator++() {
			m_it = std::find_if(std::next(m_it), std::end(m_range->m_range), std::ref(m_filter));
			return *this;
		}
		...
		iterator& operator--() {
			...
			m_it = std::find_if(rev_it{m_it}, rev_it{std::begin(m_range->m_range)}, std::ref(m_filter));
			return *this;
		}
		iterator& operator--(int) {
			auto tmp = *this;
	...
	iterator begin() const {
		...
		const auto start_it = std::find_if(std::begin(m_range), std::end(m_range), std::ref(filter));
		return iterator{*this, filter, start_it};
	}
	iterator end() const {return {*this, m_filter, std::end(m_range)};}
...
template<typename Range, typename Filter>
auto filter(Range&& range, Filter filter) {
